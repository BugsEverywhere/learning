 ...
 */
public class Quiz16_19 {

    public static void main(String[] args) {
        Quiz16_19 quiz16_19 = new Quiz16_19();
        int[] res = quiz16_19.pondSizes(new int[][]{
    ...
    public int[] pondSizes(int[][] land) {
        ...
        PriorityQueue<Integer> sizes = new PriorityQueue<>(Integer::compare);

        int[] rowDir = new int[]{-1, 0, 1};
        int[] colDir = new int[]{-1, 0, 1};

        for (int m = 0; m < land.length; m++) {
            for (int n = 0; n < land[0].length; n++) {
                if (land[m][n] != 0) {
                    continue;
                }
                int size = 0;
                ...
                //todo: BFS时一定要注意，将点加入queue时就要清除标记
                land[m][n] = -1;
                while (queue.size() > 0) {
                    int[] node = queue.poll();
                    size++;
                    ...
                    for (int i = 0; i < rowDir.length; i++) {
                        for (int j = 0; j < colDir.length; j++) {
                            if (rowDir[i] == 0 && colDir[j] == 0) {
                                //跳过自己
                                continue;
                            }
                            int rowIndex = node[0] + rowDir[i];
                            int colIndex = node[1] + colDir[j];
                            if (rowIndex < 0 || colIndex < 0 || rowIndex >= land.length || colIndex >= land[0].length) {
                                continue;
                            }
                            if (land[rowIndex][colIndex] == 0) {
                                //todo: BFS时一定要注意，将点加入queue时就要清除标记
                                queue.offer(new int[]{rowIndex, colIndex});
                                land[rowIndex][colIndex] = -1;
                            }
                        }
                    }
                }
