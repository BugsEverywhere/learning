...
public class Quiz378_notfinish {
    ...
    // 再说说为什么要这样做，首先我们要找的数是矩阵中第k大的数，而且矩阵中一定有这么一个第k大的数，所以反过来说，如果找到一个这么一个数x，矩阵中小于等于（注意一定是小于等于，光小于还不行）
    // 他的数的个数是k，那么这个x一定属于这个矩阵。
    // 然后check方法最终就能帮我们定位到这么一个数x，也就是迭代到某一个mid时，一定会有count(matrixElement<=mid)==k，剩下的是我们如何让right，left逼近这个k。
