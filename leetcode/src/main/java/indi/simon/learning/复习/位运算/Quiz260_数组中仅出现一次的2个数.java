...
public class Quiz260_数组中仅出现一次的2个数 {
    ...
    public int[] singleNumber(int[] nums) {
        ...
        // 假设让xorsum仅保留最后一位1，其他全为0，得到整数separator，作为分类器（其实取任何一位为1的整数都可以，
        // 影响的只不过是怎么获取到这个separator的问题，取最后一个1最方便），那么在所有的nums中，该位为1的数是type1，
        // 该位为0的数是type2，a和b肯定分属于type1和type2，这样最终的异或出来的结果才会使该位为1.
        // 接下来就是将所有数分成type1和type2两大类，大类中分别将所有数进行异或运算，这样就得到最终type1中的a和type2中的b了

        // todo: 这种技巧了解一下，与自己的相反数进行与运算就能得到最后一位为1的数这种操作。
        int separator = (xorsum & (-xorsum));
        int type1 = 0, type2 = 0;
        for (int num : nums) {
            if ((num & separator) != 0) {
